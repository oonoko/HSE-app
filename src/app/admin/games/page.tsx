...
export default function AdminGamesPage(){
 ...
 return <div className="app-container admin-page page-enter"><div className="page-heading-row"><div><span className="eyebrow">МЭДЛЭГ СЭРГЭЭХ</span><h1>{user.is_super_admin?'Тоглоом үүсгэх':'Тоглоом'}</h1><p>{user.is_super_admin?'Бэлэн template ашиглан тоглоомоо хурдан бэлдэнэ.':'Тоглоомын жагсаалт болон тайлан. Тоглоом удирдах эрх зөвхөн ерөнхий админд байна.'}</p></div>{user.is_super_admin&&<button className="btn-primary" onClick={()=>building?closeBuilder():startNew()}><Icon name={building?'chevron':'plus'} size={18}/>{building?'Жагсаалт':'Шинэ тоглоом'}</button>}</div>
